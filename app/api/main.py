...
async def health_check():
    ...
    try:
        ...
        # Only check external services if credentials are available
        if settings.tixr_cpk and settings.klaviyo_api_key:
            try:
                detailed_health = await integration_service.health_check()
                health_status["external_services"] = detailed_health.get("components", {})
                
                # Update overall status based on external services
                if detailed_health.get("status") == "unhealthy":
                    health_status["status"] = "degraded"
                    health_status["message"] = "External services unavailable"
                    
            except Exception as e:
                health_status["status"] = "degraded"
                health_status["external_services_error"] = str(e)
                health_status["message"] = "External services check failed"
        else:
            health_status["message"] = "Basic health check - external services not configured"
